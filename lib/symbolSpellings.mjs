...
/**
 * Our spelling to Tiingo's (#553 COWORK #55 §2). Dashed, and a preferred series
 * written "-P<series>" gains a second dash: EP-PC -> EP-P-C, FITB-PM ->
 * FITB-P-M, MER-PK -> MER-P-K. Measured, not assumed: Tiingo's metadata
 * endpoint missed exactly those three as spelled here and answered 200 for the
 * "-P-" form, which is also how its supported-tickers file lists every series
 * under those roots (CODE-B #48). A RULE rather than a three-row table, for the
 * reason symbolSpellings gives above: the next preferred in the universe is
 * covered without anyone remembering to add a row.
 *
 * A bare class letter (BRK-B) has no letter after it, and a one-letter class
 * named P ("XYZ-P") has none either, so neither is touched.
 *
 * @param {string} s @returns {string}
 */
export const toTiingo = (s) => toDashed(s).replace(/-P([A-Z])$/, "-P-$1");
