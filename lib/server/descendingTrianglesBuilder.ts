...
const DESCENDING_WAIT_STEP_MS = 300;
const DESCENDING_MAX_WAIT_MS = 12_000;

function descendingSleep(ms: number) {
 ...
 * lock with NOTHING cached -- right after DESCENDING_REDIS_TTL_SECONDS lapses,
 * after an eviction or outage, or after a cache-key version bump, when the key is
 ...
 * series; readDescendingCache is a plain GET with no hydration. But the value is
 ...
 */
async function waitForDescendingPayload() {
  if (!redis) return null;

  const startedAt = Date.now();

  while (Date.now() - startedAt < DESCENDING_MAX_WAIT_MS) {
    await descendingSleep(DESCENDING_WAIT_STEP_MS);

    try {
      if (await redis.exists(DESCENDING_REDIS_KEY)) return await readDescendingCache();
    } catch {
      return null;
    }
  }
...
async function fetchHistory(symbol: string, days: number): Promise<Point[]> {
  const pts = await getDailyHistory(symbol, { caller: "plays-descending" });
...
export async function getDescendingTrianglesData(
  ...
  if (!lockToken) {
    ...
    // second full build beside it -- see waitForDescendingPayload.
    //
    ...
    if (!forceRefresh && !debugSymbol) {
      const published = await waitForDescendingPayload();
