...
1. **The known-good ceiling is 4.3 MB** — that is what writes successfully
   today. The real Upstash limit is somewhere above it; we don't know where.
   Full coverage is a 29% step up. The losing outcome is bad and silent:
   `writePickersCache` catches and swallows, so an oversized value simply isn't
   cached and every request rebuilds from live FMP.

2. **`sections` duplicates chartPoints.** `signalRecords` is only 1.8 MB of the
   4.3 MB total; most of the other 2.4 MB is section items carrying their own
   copy of points for symbols already in `signalRecords`.

### Recommended: do the safe version first

Stop guessing the ceiling — let the code find it. In `writePickersCache`, attempt
the full payload, and on failure retry with the stripped one, logging which was
used. Then ship `chartPoints` for all 260. If 5.5 MB fits, every card gets a
chart; if it doesn't, behaviour degrades to exactly what ships today.
...
Then `fullSignalRecords = signalRecords` (no strip), with the current strip kept
as the `reduced` fallback. Re-check `/api/debug/pickers-size` afterwards and
...
`macdHist` can be dropped from their series. Worth roughly 30% of their cost.
Careful: a "preset" page draws from the full universe and *can* request an
indicator overlay, so a slimmed symbol on such a page would render the panel with
no data. Check `MiniPickerCandleChart` degrades cleanly before relying on this.

### Better, but a real refactor

Deduplicate `sections` — have section items reference symbols and read points
from `signalRecords`. That frees ~2.4 MB, more than full coverage costs, so the
payload gets *smaller* while every card gains a chart. Consumers to check:
`PickerResultPage.tsx`, `app/plays/*Client.tsx`, the homepage ticker.
...
- **A push to `main` did not trigger a build.** Commit `c4d369d` (adding the
  ...
  deployed normally. If a change seems not to be live, check
  vercel.com deployments before re-debugging the code.
...
- **Job summaries are only visible if logged.** `warm-fundamentals` now
  ...
  starvation went unnoticed for so long. Runtime log retention is ~1 day.
