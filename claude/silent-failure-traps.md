...
## 13. A fetch-level `revalidate` silently overrides the page's
...
it stopped throwing `DYNAMIC_SERVER_USAGE`; trap 11).
...
## 14. `Promise.all` reports the FIRST rejection, not the only one
...
This is the same shape as trap 1 (`[]` meaning both "empty" and "failed"): a
...
## 15. When ABSENCE is the evidence, prove the producer ran

Trap 1 inverted, and harder to catch.
...
## 16. Suspicious uniformity IS the error signal

The sharpest instance of trap 15 so far, and the one that would have shipped.
