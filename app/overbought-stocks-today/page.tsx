...
export const metadata: Metadata = {
  title:
    "Overbought Stocks Today | Potential Pullback Setups | MyStockHarbor",
  description:
    "Browse overbought stocks today using live MyStockHarbor picker data. Review potential pullback setups, open each stock page, and jump straight to the chart for deeper analysis.",
  alternates: {
  ...
  openGraph: {
    ...
    description:
      "Explore overbought stocks today and potential pullback setups using live MyStockHarbor picker data.",
...
async function getOverboughtEntries(): Promise<{
  updatedAt: string | null;
  entries: OverboughtEntry[];
}> {
  try {
    ...
    const overboughtSection = sections.find((section) =>
      String(section?.title || "").toLowerCase().includes("overbought")
    );

    const items = Array.isArray(overboughtSection?.items)
      ? overboughtSection.items
      : [];

    const entries: OverboughtEntry[] = items
      .map((item) => {
        ...
        return {
          symbol,
          label:
            String(item?.note || "Overbought setup").trim() || "Overbought setup",
          stockHref,
...
export default async function OverboughtStocksTodayPage() {
  const { updatedAt, entries } = await getOverboughtEntries();

  const jsonLd = {
    "@context": "https://schema.org",
    "@type": "CollectionPage",
    name: "Overbought Stocks Today",
    url: "https://www.mystockharbor.com/overbought-stocks-today",
    description:
      "Live page showing overbought stocks today from the MyStockHarbor picker feed.",
    mainEntity: {
      "@type": "ItemList",
      itemListElement: entries.slice(0, 24).map((entry, index) => ({
        ...
        item: {
          "@type": "Thing",
          name: `${entry.symbol} overbought stock setup`,
    ...
    breadcrumb: {
      ...
      itemListElement: [
        ...
        },
        {
          ...
          position: 3,
          name: "Overbought Stocks Today",
          item: "https://www.mystockharbor.com/overbought-stocks-today",
        },
      ],
    },
  };
    ...
    >
      ...
      <div className="wrap">
        <div style={{ display: "grid", gap: 14 }}>
          ...
          <section
            style={{
              ...
              padding: 18,
              background:
                "linear-gradient(135deg, rgba(22,10,10,0.98), rgba(10,12,18,0.98))",
              boxShadow:
          ...
          >
            ...
            >
              OVERBOUGHT STOCK SCREENER PAGE
            </div>
            ...
            >
              Overbought does not automatically mean ready to fall. Some stocks
              keep trending strongly, while others pull back sharply after
              extension. Use this page as a live shortlist, then inspect trend
              strength, resistance, momentum, and price structure before making
              any decision.
            </p>
            ...
            >
              ...
              >
                ...
                >
                  {entries.length} overbought setups
                </div>
              </div>
        ...
        <section style={panelStyle}>
          ...
          >
            Stocks currently screening as overbought
          </h2>
          ...
          >
            Review the current overbought setups below, then open any stock page
            for more detail or jump straight to the chart.
          </p>

          <div className="overbought-grid" style={{ marginTop: 18 }}>
            {entries.length > 0 ? (
              entries.map((entry) => (
                <article
                ...
                >
                  ...
                  >
                    <li>{entry.label}</li>
                    <li>Review whether strength is extending or becoming stretched</li>
                  </ul>
              ...
              >
                No overbought results are currently available from the live picker feed.
              </div>
            )}
          </div>
        </section>
        ...
        <section style={panelStyle}>
          ...
          >
            What it means when a stock is overbought
          </h2>
          ...
          >
            Overbought stocks often attract attention because strong upside moves
            can eventually lead to pause, pullback, or short-term exhaustion. For
            many traders, that makes overbought scans useful for finding charts
            where strength may be becoming stretched rather than balanced.
          </p>
          ...
          >
            That does not mean every overbought stock is ready to fall. Some names
            continue trending higher for longer than expected, especially in strong
            market conditions. Traders still need to assess resistance, momentum,
            structure, and whether the chart is showing any sign of losing strength.
          </p>
          ...
          >
            ...
            inspect the chart, and decide whether the overbought condition looks
            vulnerable to a pullback or whether the stock is still trending cleanly.
          </p>
        </section>
        ...
        >
          ...
          <Link
            ...
            style={{
              ...
              padding: 16,
              background:
                "linear-gradient(180deg, rgba(22,10,10,0.92), rgba(14,8,8,0.96))",
              textDecoration: "none",
              color: "#f1f5f9",
              display: "block",
              boxShadow: "inset 0 1px 0 rgba(255,255,255,0.03)",
          ...
          >
            ...
            >
              Compare overbought pullback candidates with stocks currently showing
              broader bearish technical signal alignment.
            </div>
          </Link>
        </section>
      </div>
