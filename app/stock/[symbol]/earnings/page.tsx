...
function classifyQuarter(row: FmpEarningsRow): EarningsTone {
  const epsPct = calcPercentDifference(
    asNumber(row.epsActual),
    asNumber(row.epsEstimated)
  );
  const revenuePct = calcPercentDifference(
    asNumber(row.revenueActual),
    asNumber(row.revenueEstimated)
  );
  ...
  if (epsPct != null) {
    if (epsPct > 2) score += 1;
    else if (epsPct < -2) score -= 1;
  }

  if (revenuePct != null) {
    if (revenuePct > 1) score += 1;
    else if (revenuePct < -1) score -= 1;
  }

  if (asNumber(row.epsActual) != null) {
    score += Number(row.epsActual) > 0 ? 0.5 : -0.5;
  }
...
function scoreEarnings(args: {
  latest: FmpEarningsRow | null;
  sameQuarterLastYear: FmpEarningsRow | null;
  completedRows: FmpEarningsRow[];
}) {
  ...
  if (!latest) {
    return {
      score: 50,
      tone: "neutral" as EarningsTone,
      label: "Unavailable",
      explanation: "Structured earnings data is not available for this symbol yet.",
    };
  }
  ...
  const yoyRevenueGrowth = calcGrowth(
    revenueActual,
    asNumber(sameQuarterLastYear?.revenueActual)
  );
  ...
  for (const row of recent) {
    const tone = classifyQuarter(row);
    if (tone === "good") score += 2.5;
...
async function getEarningsData(symbol: string) {
    ...
    ? earningsJson
        .map((item) => {
          ...
          return {
            ...
            epsEstimated: asNumber(row.epsEstimated),
            revenueActual: asNumber(row.revenueActual),
            revenueEstimated: asNumber(row.revenueEstimated),
  ...
  const next = earningsRows.find((row) => {
  ...
  const grossMargin =
    matchingIncome?.grossProfit != null &&
    matchingIncome?.revenue != null &&
    matchingIncome.revenue !== 0
      ? (matchingIncome.grossProfit / Math.abs(matchingIncome.revenue)) * 100
      : null;

  const operatingMargin =
    matchingIncome?.operatingIncome != null &&
    matchingIncome?.revenue != null &&
    matchingIncome.revenue !== 0
      ? (matchingIncome.operatingIncome / Math.abs(matchingIncome.revenue)) * 100
      : null;
  ...
  const recentTrend: EarningsTrendPoint[] = completedRows.slice(0, 6).map((row) => ({
    ...
    revenueEstimated: row.revenueEstimated ?? null,
  }));
...
function metricCardStyle(tone: EarningsTone | "default" = "default") {
  ...
  const bg =
    tone === "good"
      ? "linear-gradient(135deg, rgba(34,197,94,0.10), rgba(15,23,42,0.22))"
      : tone === "weak"
        ? "linear-gradient(135deg, rgba(239,68,68,0.10), rgba(15,23,42,0.22))"
        : tone === "neutral"
          ? "linear-gradient(135deg, rgba(250,204,21,0.10), rgba(15,23,42,0.22))"
...
export default async function StockEarningsPage({ params }: Props) {
  ...
  const yoyRevenueGrowth = calcGrowth(
    revenueActual,
    data.sameQuarterLastYear?.revenueActual ?? null
  );
    ...
    breadcrumb: {
      ...
      itemListElement: [
        ...
        },
        {
          ...
          position: 2,
          name: clean,
          item: `https://www.mystockharbor.com/stock/${clean}`,
        },
        {
          "@type": "ListItem",
          position: 3,
          name: "Earnings",
          item: `https://www.mystockharbor.com/stock/${clean}/earnings`,
        },
      ],
    },
  };
    ...
    <main className="earningsPage">
      ...
      <style>{`
        ...
        .hero {
          ...
          border-radius: 28px;
          padding: 24px;
          background: linear-gradient(135deg, rgba(15,23,42,0.96), rgba(6,10,18,0.98));
        ...
        .scoreCard {
          border: 1px solid ${toneColor(score.tone)}55;
          border-radius: 22px;
          padding: 18px;
          background: linear-gradient(135deg, ${toneBg(score.tone)}, rgba(255,255,255,0.026));
        ...
        .scorePill {
          ...
          align-items: center;
          justify-content: center;
          border: 1px solid ${toneColor(score.tone)}66;
          background: ${toneBg(score.tone)};
          color: ${toneColor(score.tone)};
          border-radius: 999px;
          padding: 8px 11px;
          font-weight: 950;
          font-size: 12px;
        ...
        .scoreNeedle {
          ...
          border-radius: 999px;
          background: #f8fafc;
          border: 3px solid ${toneColor(score.tone)};
      ...
      <div className="earningsWrap">
        ...
        <section className="hero">
          <div>
            ...
            <p>
              Review the latest reported earnings for {clean}, including actual EPS,
              estimates, revenue surprise, year-over-year context, recent earnings
              consistency and a simple earnings score.
            </p>
        ...
        <section className="contentGrid">
          <div style={{ display: "grid", gap: 18 }}>
            <section className="card">
              ...
              <p>
                Latest completed report: <strong>{formatDate(latest?.date)}</strong>.
                {" "}Next expected earnings date: <strong>{formatDate(next?.date)}</strong>.
              </p>

              {!latest ? (
                <p>
                  Structured earnings data is not available for this symbol yet.
                  When FMP returns usable EPS and revenue fields, the page will show
            ...
            <section className="card">
              ...
              <p>
                The dots below simplify recent earnings into good, mixed or weak
                reads based on EPS surprise, revenue surprise and whether the report
                was profitable.
              </p>
                  ...
                  {data.recentTrend.map((item) => (
                    <div key={item.label} className="trendDot">
                      <span
                        ...
                        title={`${item.label}: ${toneLabel(item.tone)}`}
                      />
          ...
          <aside className="sideColumn">
            <section className="card">
              ...
              <ul className="bulletList">
                <li>
                  EPS surprise shows whether profit landed above or below analyst
                  expectations.
                </li>
                <li>
                  Revenue surprise shows whether demand was stronger or weaker than
                  expected.
                </li>
                <li>
                  Year-over-year growth helps separate one-quarter noise from a real
                  earnings trend.
                </li>
              </ul>
            </section>
            ...
            <section className="card">
              ...
              <p>
                Earnings matter because they test whether the company story is being
                supported by actual revenue, profit and estimate performance. A stock
                can look technically weak but recover quickly if earnings improve, or
                look technically strong but sell off if the report disappoints.
              </p>
            </section>
                ...
                <div className="yearGrid">
                  {data.yearlySummaries.map((item) => (
                    <div
                      ...
                      style={{
                        color: "#f8fafc",
                        borderColor: `${toneColor(item.tone)}55`,
                        background: toneBg(item.tone),
                      }}
                    >
                      <span>{item.year}</span>
                      <span style={{ color: toneColor(item.tone) }}>
                        {item.toneLabel}
                      </span>
                    </div>
                  ))}
            ...
            <section className="card">
              ...
              <p>
                Use this page for the earnings read, then compare it with the stock
                page and latest news to see whether price action agrees with the
                fundamental update.
              </p>
