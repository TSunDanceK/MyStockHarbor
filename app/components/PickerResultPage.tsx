...
function toneBackground(tone?: PickerTone) {
  if (tone === "green") {
    return "linear-gradient(180deg, rgba(8,24,18,0.96), rgba(6,12,18,0.98))";
  }

  if (tone === "yellow") {
    return "linear-gradient(180deg, rgba(28,24,8,0.96), rgba(8,12,18,0.98))";
  }

  if (tone === "orange") {
    return "linear-gradient(180deg, rgba(32,20,8,0.96), rgba(8,12,18,0.98))";
  }

  if (tone === "red") {
    return "linear-gradient(180deg, rgba(32,10,14,0.96), rgba(8,12,18,0.98))";
  }
  ...
  if (config.kind === "buySignals") {
    return signalRecords
      .map((record): ResultEntry | null => {
        ...
        return {
          symbol,
          note: `${score} buy signal${score === 1 ? "" : "s"}`,
          tone: "green",
          score,
          stockHref: `/stock/${encodeURIComponent(symbol)}`,
  ...
  if (config.kind === "sellSignals") {
    return signalRecords
      .map((record): ResultEntry | null => {
        ...
        return {
          symbol,
          note: `${score} sell signal${score === 1 ? "" : "s"}`,
          tone: "red",
          score,
          stockHref: `/stock/${encodeURIComponent(symbol)}`,
          chartHref: chartHrefFor(symbol, record.dashboardHref),
      ...
      {PICKER_NAV.map((item) => {
        ...
        return (
          <Link
            key={item.href}
            href={item.href}
            className={active ? "signalTab active" : "signalTab"}
            style={{
              borderColor: active ? `${colour}99` : "rgba(96,165,250,0.36)",
              background: active
                ? `linear-gradient(135deg, ${colour}2e, rgba(30,64,175,0.16))`
                : "linear-gradient(135deg, rgba(59,130,246,0.16), rgba(15,23,42,0.62))",
            }}
          >
  ...
  const jsonLd = {
    ...
    mainEntity: {
      ...
      itemListElement: entries.slice(0, 24).map((entry, index) => ({
        ...
        item: {
          "@type": "Thing",
          name: `${entry.symbol} ${config.title}`,
  ...
  return (
    <main className="pickerResultPage">
      ...
      <style>{`
        ...
        .hero {
          border: 1px solid ${toneBorder(config.tone)};
          border-radius: 28px;
          padding: 22px;
          background: ${toneBackground(config.tone)};
        ...
        .explainer {
          ...
          border-radius: 22px;
          padding: 18px;
          background: linear-gradient(180deg, rgba(15,23,42,0.78), rgba(8,13,22,0.96));
        }
                  ...
                  <div className="resultCardTop">
                    <div>
                      <div className="symbolLine">
                        <span
                          className="dot"
                          style={{ background: toneColour(entry.tone) }}
                          aria-hidden="true"
                        />
                        <h3>{entry.symbol}</h3>
                      </div>
