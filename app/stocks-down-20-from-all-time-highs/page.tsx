...
export const metadata: Metadata = {
  title:
    "Stocks Down 20% From All-Time Highs | Live Pullback Stock List | MyStockHarbor",
  description:
    "Browse stocks down 20% from all-time highs using live MyStockHarbor picker data. Review pullback setups, open each stock page, and jump straight to the chart for deeper analysis.",
  alternates: {
  ...
  openGraph: {
    ...
    description:
      "Explore stocks down 20% from all-time highs using live MyStockHarbor picker data.",
...
async function getAthPullbackEntries(): Promise<{
  updatedAt: string | null;
  entries: AthPullbackEntry[];
}> {
  try {
    ...
    const athPullbackSection = sections.find((section) =>
      String(section?.title || "").includes("All-Time Highs")
    );

    const items = Array.isArray(athPullbackSection?.items)
      ? athPullbackSection.items
      : [];

    const entries: AthPullbackEntry[] = items
      .map((item) => {
        ...
        return {
          symbol,
          label: String(item?.note || "Pullback setup").trim() || "Pullback setup",
          stockHref,
      ...
      .filter((entry): entry is AthPullbackEntry => Boolean(entry));
...
export default async function StocksDown20FromAllTimeHighsPage() {
  const { updatedAt, entries } = await getAthPullbackEntries();

  const jsonLd = {
    "@context": "https://schema.org",
    "@type": "CollectionPage",
    name: "Stocks Down 20% From All-Time Highs",
    url: "https://www.mystockharbor.com/stocks-down-20-from-all-time-highs",
    description:
      "Live page showing stocks down 20% from all-time highs from the MyStockHarbor picker feed.",
    mainEntity: {
      "@type": "ItemList",
      itemListElement: entries.slice(0, 24).map((entry, index) => ({
        ...
        item: {
          "@type": "Thing",
          name: `${entry.symbol} stock down from all-time highs`,
    ...
    breadcrumb: {
      ...
      itemListElement: [
        ...
        },
        {
          ...
          position: 3,
          name: "Stocks Down 20% From All-Time Highs",
          item: "https://www.mystockharbor.com/stocks-down-20-from-all-time-highs",
        },
      ],
    },
  };
    ...
    >
      ...
      <div className="wrap">
        <div style={{ display: "grid", gap: 14 }}>
          ...
          <section
            style={{
              ...
              padding: 18,
              background:
                "linear-gradient(135deg, rgba(24,18,8,0.98), rgba(12,12,18,0.98))",
              boxShadow:
          ...
          >
            ...
            >
              This page shows stocks currently screening as meaningful pullbacks
              from all-time highs using live MyStockHarbor picker data. It is
              built for traders who want to review names that have corrected away
              from prior highs and may be setting up near more interesting levels.
            </p>
            ...
            >
              A stock being down 20% from its high does not automatically make it
              attractive. Some pullbacks become opportunities, while others are
              simply weak charts. Use this page as a shortlist, then inspect price
              structure, trend quality, support, resistance, and momentum before
              making any decision.
            </p>
            ...
            >
              <div
                style={{
                  ...
                  background:
                    "linear-gradient(135deg, rgba(234,179,8,0.10), rgba(15,23,42,0.08))",
                }}
              >
                <div
                  style={{
                ...
                >
                  {entries.length} pullback setups
                </div>
              </div>
        ...
        <section style={panelStyle}>
          ...
          >
            Review the current pullback setups below, then open any stock page for
            more detail or jump straight to the chart.
          </p>

          <div className="ath-grid" style={{ marginTop: 18 }}>
            {entries.length > 0 ? (
              entries.map((entry) => (
                <article
                  ...
                  style={{
                    ...
                    padding: 16,
                    background:
                      "linear-gradient(180deg, rgba(18,16,8,0.96), rgba(6,10,18,0.96))",
                ...
                >
                  ...
                  >
                    ...
                    <div
                      style={{
                        ...
                        background:
                          "linear-gradient(135deg, rgba(234,179,8,0.14), rgba(202,138,4,0.08))",
                        color: "#fef3c7",
                        fontWeight: 900,
                        fontSize: 12,
                  ...
                  <MiniPickerCandleChart points={entry.chartPoints} tone="yellow" />
                  ...
                  >
                    <li>{entry.label}</li>
                    <li>Review structure before treating it as value</li>
                  </ul>
              ...
              >
                No pullback results are currently available from the live picker feed.
              </div>
            )}
          </div>
        </section>
        ...
        <section style={panelStyle}>
          ...
          >
            Stocks that are well below prior highs often attract attention because
            they may be moving into areas where risk and reward start to look more
            interesting. In some cases, a clean pullback inside a stronger trend
            can create a better entry zone than chasing strength near the highs.
          </p>
          ...
          >
            That said, a stock being down 20% is not bullish on its own. Some names
            are simply weak, broken, or still falling. Traders still need to assess
            trend quality, support, resistance, momentum, and whether the chart is
            stabilising or still deteriorating.
          </p>
          ...
          >
            ...
            inspect the chart, and decide whether the pullback looks constructive
            or whether the stock is still under pressure.
          </p>
        </section>
        ...
        >
          ...
          >
            ...
            >
              Compare pullback names with stocks currently showing stronger bullish
