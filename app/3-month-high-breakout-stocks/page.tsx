...
export const metadata: Metadata = {
  title:
    "3-Month High Breakout Stocks | Live Breakout Stock List | MyStockHarbor",
  description:
    "Browse 3-month high breakout stocks using live MyStockHarbor picker data. Review stocks pushing through recent highs, open each stock page, and jump straight to the chart for deeper analysis.",
  alternates: {
  ...
  openGraph: {
    ...
    description:
      "Explore 3-month high breakout stocks using live MyStockHarbor picker data.",
...
async function getThreeMonthBreakoutEntries(): Promise<{
  updatedAt: string | null;
  entries: ThreeMonthBreakoutEntry[];
}> {
  try {
    ...
    const threeMonthBreakoutSection = sections.find((section) =>
      String(section?.title || "").includes("3-Month High Breakout")
    );

    const items = Array.isArray(threeMonthBreakoutSection?.items)
      ? threeMonthBreakoutSection.items
      : [];

    const entries: ThreeMonthBreakoutEntry[] = items
      .map((item) => {
        ...
        return {
          symbol,
          label:
            String(item?.note || "3-month breakout setup").trim() ||
            "3-month breakout setup",
          stockHref,
...
export default async function ThreeMonthHighBreakoutStocksPage() {
  const { updatedAt, entries } = await getThreeMonthBreakoutEntries();

  const jsonLd = {
    "@context": "https://schema.org",
    "@type": "CollectionPage",
    name: "3-Month High Breakout Stocks",
    url: "https://www.mystockharbor.com/3-month-high-breakout-stocks",
    description:
      "Live page showing 3-month high breakout stocks from the MyStockHarbor picker feed.",
    mainEntity: {
      "@type": "ItemList",
      itemListElement: entries.slice(0, 24).map((entry, index) => ({
        ...
        item: {
          "@type": "Thing",
          name: `${entry.symbol} 3-month high breakout stock`,
    ...
    breadcrumb: {
      ...
      itemListElement: [
        ...
        },
        {
          ...
          position: 3,
          name: "3-Month High Breakout Stocks",
          item: "https://www.mystockharbor.com/3-month-high-breakout-stocks",
        },
      ],
    },
  };
    ...
    >
      ...
      <div className="wrap">
        <div style={{ display: "grid", gap: 14 }}>
          ...
          <section
            style={{
              ...
              padding: 18,
              background:
                "linear-gradient(135deg, rgba(20,14,8,0.98), rgba(10,12,18,0.98))",
              boxShadow:
          ...
          >
            ...
            >
              This page shows stocks currently screening for 3-month high
              breakout setups using live MyStockHarbor picker data. It is built
              for traders who want to review names pushing through recent highs,
              where momentum, relative strength, and trend continuation may be
              starting to improve.
            </p>
            ...
            >
              A move into a 3-month high can highlight emerging strength earlier
              than an all-time high breakout, but it still needs chart context.
              Some breakouts continue cleanly, while others fade quickly. Use
              this page as a live shortlist, then inspect structure, extension,
              volume, and follow-through before making any decision.
            </p>
        ...
        <section style={panelStyle}>
          ...
          >
            Review the current 3-month high breakout setups below, then open any
            stock page for more detail or jump straight to the chart.
          </p>

          <div className="three-month-breakout-grid" style={{ marginTop: 18 }}>
            {entries.length > 0 ? (
              entries.map((entry) => (
                <article
              ...
              >
                No 3-month high breakout results are currently available from the
                live picker feed.
              </div>
            )}
          </div>
        ...
        <section style={panelStyle}>
          ...
          >
            A 3-month high breakout can highlight improving strength before a
            stock reaches all-time highs. For many traders, this makes it a useful
            early momentum signal when price begins pushing through recent
            resistance with improving participation.
          </p>
          ...
          >
            That does not mean every breakout is high quality. Some moves are
            late, extended, or vulnerable to fast reversals. Traders still need
            to judge structure, volume, acceleration, and whether the breakout is
            clean enough to support continuation.
          </p>
        ...
        >
          ...
          <Link
            ...
            style={{
              ...
              padding: 16,
              background:
                "linear-gradient(180deg, rgba(20,14,8,0.92), rgba(12,10,8,0.96))",
              textDecoration: "none",
              color: "#f1f5f9",
              display: "block",
              boxShadow: "inset 0 1px 0 rgba(255,255,255,0.03)",
          ...
          >
            <div style={{ fontSize: 18, fontWeight: 950 }}>
              Explore all-time high breakouts
            </div>
            <div
              style={{
                marginTop: 8,
            ...
            >
              Compare 3-month breakout names with stocks already pushing into
