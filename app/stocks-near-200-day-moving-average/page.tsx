...
async function getMa200Entries(): Promise<{
  ...
  try {
    ...
    const ma200Section = sections.find((section) =>
      String(section?.title || "").toLowerCase().includes("200")
    );

    const sectionItems = Array.isArray(ma200Section?.items) ? ma200Section.items : [];
    ...
    for (const item of sectionItems) {
      ...
      const timeframe: "Daily" | "Weekly" =
        /weekly/i.test(note) ? "Weekly" : "Daily";
