...
export async function GET(req: NextRequest) {
  ...
  // ── THE FILING PHASE MOVED TO ITS OWN JOB (#535 COWORK #10/#12) ─────────
  // Reading a lagging filer's period from the filing itself now runs in
  // app/api/jobs/sec-filings on its own schedule (2-hourly in season, nightly
  // after the daily index), with its own state hash. This loop keeps only the
  // rule that it must not overwrite a period that job filled: see keepFilled.
  ...
  for (const { symbol, reason } of work) {
    ...
    try {
      ...
      //
      // DECIDED FROM THE STORED SET ITSELF, not from a manifest field: the
      // filing job keeps its own state (secFilingJob), and the set's `ff`/`lg`
      // stamp already names the filing and its period.
      const freshNewest = newestStoredEnd(fresh) ?? "";
