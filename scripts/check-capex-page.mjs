...
async function suite(P, pageCode) {
  ...
  const c = P.buildContractRows([{ ticker: "LMT", amount: 50e9, entities: [{ name: "LOCKHEED MARTIN CORPORATION" }, { name: "SIKORSKY AIRCRAFT CORPORATION" }, { name: "LOCKHEED MARTIN CORPORATION" }] }, { ticker: "GD", amount: 25e9, entities: [{ name: "ELECTRIC BOAT CORPORATION" }] }, { ticker: "ZZZ", amount: 1e9, entities: [] }], 15, (t) => names[t] ?? "");
  ok("contract bars scale to the largest, in dollars", c[0].barPct === 100 && c[1].barPct === 50 && c[0].amount === "$50.0bn" && c[0].entities.length === 2 && c[0].records === 3);
