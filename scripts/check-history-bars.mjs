...
if (!barMissing.length) {
  const decls = raw
    .split("\n")
    .filter((l) =>
      /^(let historyStaleNewestCount|let historyFreshNewestCount|const historyStaleNewest |let historyNewestBarSeen|const MAX_DIAGNOSTIC_SYMBOLS|let historyForcedRefetchFailures|const historyForcedRefetchFailureSymbols|const historyForcedFailureReasons|const historyNewestBarStamps|let historyRanOutOfTime|let historyDeferredOutOfTime|export const HISTORY_MAX_BAR_AGE_WEEKDAYS)/.test(l)
    )
