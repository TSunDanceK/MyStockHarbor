...
const MUTANTS = [
  ...
  [MOD, "S17", "the habit line read back off the store's estimate — two medians for one number",
    "  const m = median(lags);\n  return m == null ? null : { medianLagDays: m, fromPeriods: lags.length };",
    "  const m = rec?.next?.kind === \"date\" ? rec.next.medianLagDays : null;\n  return m == null ? null : { medianLagDays: m, fromPeriods: lags.length };"],
  [MOD, "S18", "the sample size inflated to every stored event, including the ones the median dropped",
    "fromPeriods: lags.length };",
    "fromPeriods: Array.isArray(rec?.events) ? rec.events.length : lags.length };"],
  [MOD, "S19", "a period end printed beside 'not in the next 30 days', handing the reader the arithmetic",
    "      evidence: habit ? evidenceFor(habit.medianLagDays, habit.fromPeriods, null, rec) : compact([lastFiled(rec)]),",
    "      evidence: habit ? evidenceFor(habit.medianLagDays, habit.fromPeriods, rec?.nextPeriodEnd ?? null, rec) : compact([lastFiled(rec)]),"],
