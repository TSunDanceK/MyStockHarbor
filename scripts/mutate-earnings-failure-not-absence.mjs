...
const MUTANTS = [
  ...
  },
  {
    ...
    file: CAL,
    from: `    quotedEveryCandidate && !anyCapped && !anyFailed && !contradictsItsOwnCandidates;`,
    to: `    quotedEveryCandidate && !anyCapped && !contradictsItsOwnCandidates;`,
  },
  {
    id: "F2   the write guard inverted (writes empty when candidates > 0)",
    file: CAL,
    from: `  if (!contradictsItsOwnCandidates) {\n    // Materialise what we have, so the next render -- and Show more -- read it\n    // back instead of re-quoting.\n    await writeDayItemsCache(date, items);\n  }`,
    to: `  await writeDayItemsCache(date, items);`,
  },
  {
    id: "F2b  the contradiction dropped from the completeness test",
    file: CAL,
    from: `    quotedEveryCandidate && !anyCapped && !anyFailed && !contradictsItsOwnCandidates;`,
    to: `    quotedEveryCandidate && !anyCapped && !anyFailed;`,
  },
  {
    id: "F3   the cache read reverted to truthiness",
    file: CAL,
    from: `    if (cachedItems && (cachedItems.length > 0 || totalCandidates === 0)) {`,
    to: `    if (cachedItems) {`,
  },
  {
