...
for (const field of ["open", "dayHigh", "dayLow"]) {
  ...
  check(`${field}: read off the quote row`, new RegExp(`${field}: num\\(row\\.${field}\\)`).test(pool));
  ...
  check(`${field}: parsed back on read`, new RegExp(`${field}: num\\(row\\.${field}\\)`).test(pool));
}
...
check(
  "warns when quotes were fetched and NONE carried an open",
  /if \(pxRefreshed > 0 && openCarried === 0\)/.test(pool)
);
check(
  "...and does NOT warn when no quotes were fetched at all",
  /pxRefreshed > 0 &&/.test(pool),
  "absence needs the producer to have run"
);
...
check(
  ...
  /const row = \(Array\.isArray\(json\) \? json\[0\] : json\)[\s\S]{0,400}open: num\(row\.open\)/.test(pool)
);
